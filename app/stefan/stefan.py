...
def prepare_data(df):
    features = df[['rsi', 'macd', 'macd_signal', 'upper_band', 'middle_band', 'lower_band', 'sma', 'atr', 'stoch']]
...
def check_signals(df, model, scaler):
    if not hasattr(scaler, 'scale_'):
        ...
        return None
    latest_data = df.iloc[-1][['rsi', 'macd', 'macd_signal', 'upper_band', 'middle_band', 'lower_band', 'sma', 'atr', 'stoch']].values.reshape(1, -1)
