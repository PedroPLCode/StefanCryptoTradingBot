...
def check_day_trade_buy_signal(df):
...
def check_day_trade_sell_signal(df):
    latest_data = df.iloc[-1]
    mfi = talib.MFI(df['high'], df['low'], df['close'], df['volume'], timeperiod=5)
...
def run_day_trading_logic(bot_settings):
    try:
        with current_app.app_context():
            if bot_settings and bot_settings.bot_running:
                ...
                buy_signal = check_day_trade_buy_signal(df)
                sell_signal = check_day_trade_sell_signal(df)
                ...
                logger.trade(f"Day Trading bot {bot_settings.id}: Aktualna cena: {current_price}, Trailing stop loss: {trailing_stop_price}")
