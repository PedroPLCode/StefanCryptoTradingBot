...
def calculate_ta_cci(df, bot_settings):
    ...
    """
    df["cci"] = talib.CCI(
        df["high"], df["low"], df["close"], timeperiod=bot_settings.cci_timeperiod
    )
...
def calculate_ta_adx(df, bot_settings):
    ...
    df["adx"] = talib.ADX(
        df["high"], df["low"], df["close"], timeperiod=bot_settings.adx_timeperiod
    )
...
def calculate_ta_atr(df, bot_settings):
    ...
    """
    df["atr"] = talib.ATR(
        df["high"], df["low"], df["close"], timeperiod=bot_settings.atr_timeperiod
    )
...
def calculate_ta_di(df, bot_settings):
    ...
    df["plus_di"] = talib.PLUS_DI(
        df["high"], df["low"], df["close"], timeperiod=bot_settings.di_timeperiod
    )
...
def check_ta_trend(df, bot_settings):
    ...
    adx_trend = float(latest_data["adx"]) > float(
        bot_settings.adx_strong_trend
    ) or float(latest_data["adx"]) > float(avg_adx)
    ...
    is_strong_plus_di = float(latest_data["plus_di"]) > float(
        bot_settings.adx_weak_trend
    )
    is_strong_minus_di = float(latest_data["minus_di"]) > float(
        bot_settings.adx_weak_trend
    )
    ...
    horizontal = (
        ...
        or avg_adx < float(bot_settings.adx_weak_trend)
    ...
    if uptrend:
        logger.trade(
            f"Bot {bot_settings.id} {bot_settings.strategy} have BULLISH UPTREND"
        )
        return "uptrend"
    elif downtrend:
        logger.trade(
            f"Bot {bot_settings.id} {bot_settings.strategy} have BEARISH DOWNTREND"
        )
        return "downtrend"
    elif horizontal:
        logger.trade(
            f"Bot {bot_settings.id} {bot_settings.strategy} have HORIZONTAL TREND"
        )
        return "horizontal"
...
def calculate_take_profit(current_price, bot_settings):
    ...
    logger.trade(
        f"bot {bot_settings.id} {bot_settings.strategy} "
...
def calculate_stop_loss(current_price, trailing_stop_price, bot_settings):
    ...
    try:
        ...
        send_admin_email(
            f"Bot {bot_settings.id} ValueError in update_stop_loss", str(e)
        )
...
def calculate_atr_trailing_stop_loss(
    ...
    try:
        ...
        dynamic_trailing_stop = current_price * (
            1 - (bot_settings.trailing_stop_atr_calc * atr / current_price)
        )
        ...
        logger.error(
            f"Bot {bot_settings.id} ValueError in update_atr_trailing_stop_loss: {str(e)}"
        )
        send_admin_email(
            f"Bot {bot_settings.id} ValueError in update_atr_trailing_stop_loss", str(e)
        )
        ...
        logger.error(
            f"Bot {bot_settings.id} Exception in update_atr_trailing_stop_loss: {str(e)}"
        )
        send_admin_email(
            f"Bot {bot_settings.id} Exception in update_atr_trailing_stop_loss", str(e)
        )
